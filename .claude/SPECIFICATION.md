...
**注意**: この仕様書は実際のコード実装を優先して作成されています。元の仕様書との相違点は「⚠️ 仕様差異」として明記しています。
...
**例**: 性格補正値を元仕様の`1.11/0.91`に変更した場合
→ 該当する「⚠️ 仕様差異」の注記を削除または更新
...
3. **差異を明示**: 元仕様と異なる場合は「⚠️ 仕様差異」で明記する
4. **更新履歴を記録**: ファイル末尾の「最終更新」セクションを更新する
...
⚠️ **仕様差異**: 元仕様書では「×1.11 boost / ×0.91 reduction」と記載されていましたが、実装では`0.9`と`1.075`を使用しています。
...
⚠️ **仕様差異**: 元仕様書では「×1.18 to ×1.5」と記載されていましたが、実装では加算方式（0.05, 0.07, 0.14）を使用しています。
...
⚠️ **重要**: この式は元仕様書には詳細記載がなく、コードから判明しました。
...
この仕様書は、実際のコード実装を元に作成されています。元の仕様書と異なる点（性格補正値、サブスキルボーナス計算方式等）は、すべて実装コードを優先しています。
